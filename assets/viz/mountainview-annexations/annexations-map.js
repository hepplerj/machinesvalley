...
export default class AnnexationsMap extends Visualization {
  ...
  drawMap(year) {
    ...
    try {
      // Filter data by year
      const features = topojson.feature(this.data, this.data.objects[`annex_${year}`]).features;
      ...
      const resetView = () => {
        const bounds = pathGenerator.bounds(topojson.feature(this.data, this.data.objects[`annex_${year}`]));
