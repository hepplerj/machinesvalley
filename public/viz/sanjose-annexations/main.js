(() => {
  ...
  function transform_default(transform2) {
    if (transform2 == null) return identity_default2;
  ...
  // ns-hugo:/Users/jheppler/Dropbox/30-39 Projects/30.01 Dev/machinesvalley-web/htdoc/assets/viz/common/visualization.js
  var Visualization = class {
    constructor(id2, data, dim, margin) {
      this.data = data;
      this.margin = margin;
      this.svg = select_default2(id2);
      ...
      this.viz = this.svg.append("g").attr("transform", `translate(${margin.left},${margin.top})`);
    }
  };

  // ns-hugo:/Users/jheppler/Dropbox/30-39 Projects/30.01 Dev/machinesvalley-web/htdoc/assets/viz/sanjose-annexations/annexations-map.js
  var AnnexationsMap = class extends Visualization {
    constructor(id2, data, dim) {
      const margin = {
        top: 10,
        right: 10,
        bottom: 10,
      ...
      super(id2, data, dim, margin);
      this.width = 860;
      this.height = 500;
      this.currentYear = 1850;
      this.initializeMap(id2);
      this.initializeSvgLayers();
      this.ready(null, data);
    }
    initializeMap(id2) {
      this.mymap = L.map(id2, {
        center: [37.31, -121.9],
        zoom: 11,
        dragging: true,
    ...
    initializeSvgLayers() {
      this.svg = select_default2(this.mymap.getPanes().overlayPane).append("svg").attr("class", "leaflet-zoom-hide");
      this.g = this.svg.append("g").attr("class", "leaflet-zoom-hide");
      this.loading = this.svg.append("text").attr("x", this.width / 2).attr("y", this.height / 2).attr("text-anchor", "middle").text("Loading the map ...");
    }
    ready(error, data) {
      if (error) {
    ...
    drawMap(year) {
      ...
      const pathGenerator = path_default().projection(transform2);
      try {
        const features = feature_default(this.data, this.data.objects[`anex_${year}`]).features;
        const paths = this.g.selectAll("path").data(features).enter().append("path").attr("class", "annexation-area").attr("d", pathGenerator);
        const resetView = () => {
          const bounds = pathGenerator.bounds(feature_default(this.data, this.data.objects[`anex_${year}`]));
          const topLeft = bounds[0];
          const bottomRight = bounds[1];
          this.svg.attr("width", bottomRight[0] - topLeft[0]).attr("height", bottomRight[1] - topLeft[1]).style("left", `${topLeft[0]}px`).style("top", `${topLeft[1]}px`);
        ...
        this.g.append("text").attr("class", "error-message").attr("x", this.width / 2).attr("y", this.height / 2).text("Error drawing map data");
      }
    }
    renderDate(year) {
  ...
  var MapController = class {
    constructor() {
      __publicField(this, "handleYearButtonClick", (event) => {
    ...
    initialize() {
      fetch("/data/sj_annexations.json").then((response) => {
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        return response.json();
      }).then((annexationsData) => {
        this.map = new AnnexationsMap("map", annexationsData, {
          width: 1e3,
          height: 525
        });
        this.map.render();
