(() => {
  ...
  // ns-hugo:/Users/jheppler/Dropbox/30-39 Projects/30.01 Dev/machinesvalley-web/htdoc/assets/viz/mountainview-annexations/annexations-map.js
  var AnnexationsMap = class extends Visualization {
    ...
    drawMap(year) {
      ...
      try {
        const features = feature_default(this.data, this.data.objects[`annex_${year}`]).features;
        const paths = this.g.selectAll("path").data(features).enter().append("path").attr("class", "annexation-area").attr("d", pathGenerator);
        const resetView = () => {
          const bounds = pathGenerator.bounds(feature_default(this.data, this.data.objects[`annex_${year}`]));
  ...
  var MapController = class {
    ...
    initialize() {
      fetch("/data/mountainview.json").then((response) => {
        if (!response.ok) {
